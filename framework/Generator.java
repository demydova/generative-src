...
//Description: 	class for generic generating of input parameters
//Version		00.22 17.03.2017
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
...
public class Generator {
	...
	// function for filling of argument list with random values depending on the
	// parameter types
	public void fill_arguments(Object[] v_arg, Class[] v_params, boolean[] not_zero, String[][] extremum,
			String[][] array_length) {
		...
		for (int i = 0; i < v_arg.length; i++) {
			if (not_zero == null)
				not_zero[i] = false;
			if (extremum[i][0] == null)
				extremum[i][0] = "0";
			if (extremum[i][1] == null)
				extremum[i][1] = "100";
			if (array_length[i][0] == null)
				array_length[i][0] = "1";
			if (array_length[i][1] == null)
				array_length[i][1] = "20";
		}
		...
		for (int i = 0; i < v_arg.length; i++) {
			...
			case "boolean":
				v_arg[i] = new Random().nextBoolean();
				break;

			// Random generation of char values
			case "char":
				v_arg[i] = (char) (new Random().nextInt(26) + 'a');
				break;
			...
			case "String":
				...
				if (!array_length[i][0].equals(""))
					len_min = Integer.parseInt(array_length[i][0]);
				if (!array_length[i][1].equals("")) {
					len_max = Integer.parseInt(array_length[i][1]);
			...
			case "int[]":
				...
				if (!extremum[i][0].equals(""))
					v_min = Integer.parseInt(extremum[i][0]);
				if (!extremum[i][1].equals(""))
					v_max = Integer.parseInt(extremum[i][1]);
				...
				// the first and last element of array is set with extremum
				// value
				if (ar1.length > 1) {
					Array.setInt(ar1, 0, v_min);
					Array.setInt(ar1, ar1.length - 1, v_max);
				}
				// if the generated array contains only 1 element, let it be the
				// minimum one
				if (ar1.length == 1) {
					Array.setInt(ar1, 0, v_min);
				}
	...
	// the method that delete one element from array or string for simplifying of parameter in the procedure for looking for minimal failing case
	public static Object delete_random_element(Class[] v_params_testfunc, Object v_in_object, int element)
			throws InstantiationException, IllegalAccessException {
		Object result = v_in_object;
		int del;
		//check if the given parameter is array
		if (v_params_testfunc[element].getSimpleName().contains("[]")) {
			//if array - define new array and skip one element
			Object res = Array.newInstance(v_params_testfunc[element].getComponentType(),
					Array.getLength(v_in_object) - 1);
			...
			//if parameter is string - replace one symbol to ""
			result = ((String) v_in_object).replaceFirst(
					"" + ((String) v_in_object).charAt(new Random().nextInt(((String) v_in_object).length() - 1)), "");
		}
		return result;
	}
}
